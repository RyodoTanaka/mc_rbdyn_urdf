...
Eigen::Matrix3d RPY(const double &r, const double &p, const double &y) {
  ...
  m << ca1 * cb1, ca1 * sb1 * sc1 - sa1 * cc1, ca1 * sb1 * cc1 + sa1 * sc1,
      sa1 * cb1, sa1 * sb1 * sc1 + ca1 * cc1, sa1 * sb1 * cc1 - ca1 * sc1, -sb1,
      cb1 * sc1, cb1 * cc1;
  return m.transpose();
}
...
Eigen::Matrix3d RPY(const std::vector<double> rpy) {
  if (rpy.size() != 3) {
    std::cerr << "Cannot convert RPY vector of size " << rpy.size()
              << " to matrix" << std::endl;
...
inline Eigen::Matrix3d readInertia(const tinyxml2::XMLElement &dom) {
  Eigen::Matrix3d m;
  m << dom.DoubleAttribute("ixx"), dom.DoubleAttribute("ixy"),
      dom.DoubleAttribute("ixz"), dom.DoubleAttribute("ixy"),
      dom.DoubleAttribute("iyy"), dom.DoubleAttribute("iyz"),
      dom.DoubleAttribute("ixz"), dom.DoubleAttribute("iyz"),
      dom.DoubleAttribute("izz");
  return m;
}

rbd::Joint::Type rbdynFromUrdfJoint(const std::string &type,
                                    bool hasSphericalSuffix = false) {
  if (type == "revolute")
...
sva::PTransformd originFromTag(const tinyxml2::XMLElement *dom) {
  ...
  if (dom) {
    ...
    if (originDom) {
      Eigen::Vector3d T =
          attrToVector(*originDom, "xyz", Eigen::Vector3d(0, 0, 0));
...
sva::PTransformd originFromTag(const tinyxml2::XMLElement &root,
                               const std::string &tagName) {
  return originFromTag(root.FirstChildElement(tagName.c_str()));
}

std::string parseMultiBodyGraphFromURDF(
    URDFParserResult &res, const std::string &content,
    const std::vector<std::string> &filteredLinksIn, bool transformInertia,
    const std::string &baseLinkIn, bool withVirtualLinks,
  ...
  // Extract link elements from the document, remove filtered links
  {
    ...
    while (link) {
      ...
      if (std::find(filteredLinks.begin(), filteredLinks.end(), linkName) ==
          filteredLinks.end()) {
  ...
  if (links.size() == 0) {
    std::cerr << "Failed to extract any link information from the URDF, "
                 "parsing will stop now"
              << std::endl;
    return "";
  }

  std::string baseLink =
      baseLinkIn == "" ? links[0]->Attribute("name") : baseLinkIn;
  ...
  for (tinyxml2::XMLElement *linkDom : links) {
    ...
    if (not isVirtual) {
      tinyxml2::XMLElement *originDom =
          inertialDom->FirstChildElement("origin");
      tinyxml2::XMLElement *massDom = inertialDom->FirstChildElement("mass");
      tinyxml2::XMLElement *inertiaDom =
          inertialDom->FirstChildElement("inertia");
    ...
    for (tinyxml2::XMLElement *child = linkDom->FirstChildElement("visual");
      ...
      if (geometryDom) {
        ...
        if (meshDom) {
          ...
          // std::cerr << "Warning: only mesh geometry is supported, visual
          // element has been ignored" << std::endl;
        }
  ...
  // Extract joint elements from the document, remove joints that link with
  // filtered links
  {
    ...
    while (joint) {
      ...
      std::string child_link =
          joint->FirstChildElement("child")->Attribute("link");
      if (std::find(filteredLinks.begin(), filteredLinks.end(), child_link) ==
              filteredLinks.end() &&
          std::find(filteredLinks.begin(), filteredLinks.end(), parent_link) ==
              filteredLinks.end()) {
  ...
  for (tinyxml2::XMLElement *jointDom : joints) {
    ...
    if (originDom) {
      Eigen::Vector3d staticT =
          attrToVector(*originDom, "xyz", Eigen::Vector3d(0, 0, 0));
      Eigen::Matrix3d staticR =
          RPY(attrToList(*originDom, "rpy", {0.0, 0.0, 0.0}));
    ...
    rbd::Joint::Type type = rbdynFromUrdfJoint(
        jointType,
        (jointName.length() >= sphericalSuffix.length() &&
         jointName.substr(jointName.length() - sphericalSuffix.length(),
                          sphericalSuffix.length()) == sphericalSuffix));
    ...
    res.mbg.linkBodies(jointParent, staticTransform, jointChild,
                       sva::PTransformd::Identity(), jointName);
    ...
    auto check_limit = [&j](const std::string &name,
                            const std::vector<double> &limit) {
      if (limit.size() != static_cast<size_t>(j.dof())) {
        std::cerr << "Joint " << name << " limit for " << j.name()
                  << ": size missmatch, expected: " << j.dof()
...
URDFParserResult
rbdyn_from_urdf(const std::string &content, bool fixed,
                const std::vector<std::string> &filteredLinksIn,
                bool transformInertia, const std::string &baseLinkIn,
                bool withVirtualLinks, const std::string sphericalSuffix) {
  URDFParserResult res;

  std::string baseLink = parseMultiBodyGraphFromURDF(
      res, content, filteredLinksIn, transformInertia, baseLinkIn,
